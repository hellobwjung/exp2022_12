...
int main() {
  ...
  for (int TC = 0; TC < 10; TC++) {
    ...
    clock_t start = clock();
    test(user_data);
    // 1. user_data를 내림차순 정렬해야 함.
    SCORE += clock() - start;

    for (int i = 0; i < DATA_SIZE - 1; i++) {
      if (user_data[i] < user_data[i + 1]) { // 오름차순이면,벌점,
        SCORE = 100000000;                   // 내림차순이어야
        break;
      }
    }
    for (int i = 0; i < 100; i++) {                   // 100 개 data를
      int k = myrand() % DATA_SIZE, flag = 0;         // 마무렇게나 골라
      for (int j = 0; j < DATA_SIZE; j++) {
        if (user_data[j] == data[k]) {   // 같으면 ok
          flag = 1;
          break;
        }
      }
      if (flag == 0)
        SCORE = 100000000;        // 다르면 벌점 !!!
    }
  }
