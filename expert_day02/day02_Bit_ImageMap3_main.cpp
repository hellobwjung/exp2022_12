...
int cnt[CHARN] = { 79, 13, 24, 48, 129, 20, 25, 51, 76, 3, 22, 18, 32, 54, 110,
    10, 1, 55, 40, 95, 16, 8, 34, 2, 29, 1 };
char enc_str[STRN];
char charlist[996];
int wordlen[WORDN];
char words[WORDN][8];
...
void build2() {
  ...
  while (1) {
    k = myrand() % WORDN;
    if (len + wordlen[k] >= STRN)
      break;
    for (i = 0; i < wordlen[k]; i++)
      str[len++] = words[k][i];
    str[len++] = ' ';
  }
  for (i = 0; i < STRN; i++)
